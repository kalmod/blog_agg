...
const selectFeedFollowUser = `-- name: SelectFeedFollowUser :many
select id, feed_id, user_id, created_at, updated_at from feed_follow where user_id = $1
`

func (q *Queries) SelectFeedFollowUser(ctx context.Context, userID uuid.UUID) ([]FeedFollow, error) {
	rows, err := q.db.QueryContext(ctx, selectFeedFollowUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
